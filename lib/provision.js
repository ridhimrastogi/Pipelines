...
class DigitalOceanProvider
{
	...
	async createDroplet (dropletName, region, imageName, size_slug )
	{
		...
		try {
			var result = await this.dropletInfo(serverInfos[dropletName].id);
		...
		var data =
		{
			...
			"size":size_slug,
			"image":imageName,
			"ssh_keys":[process.env.VM_Pub_Key_FingerPrint],
			"backups":false,
			"ipv6":false,
			"user_data":null,
	 ...
	 async dropletInfoWait(id){
		...
		do {
			try {					
				...
				console.log(`${droplet.name}: ${address}`);
				ipV4isErr = false;	
...
{	
	...
	try {
		serverInfos = JSON.parse(fs.readFileSync("cm/serverInfos_DigitalOcean.json", 'utf8'));  
	} catch (error) {
